Total output lines: 697
Output:
# คู่มือวิศวกรรม Solar Grid-Connected ฉบับรวมเล่ม

> เอกสารฉบับนี้รวมเนื้อหาจากทุกบทไว้ในหน้าเดียวสำหรับ GitHub. ไฟล์แยกรายบทคงอยู่เพื่อการแก้ไขและอ้างอิงแบบเฉพาะเรื่อง.

## สารบัญ

1. บทนำ · PCC · CT · PT/VT/CVT
2. CT Burden · CT Saturation
3. Metering · Protection · Reverse Power · Zero Export
4. การออกแบบ 400 V · 22 kV · 115 kV
5. Utility Requirements · Approval · Commissioning · O&M
6. Case Studies · Checklists · References

---

<!-- SOURCE: README.md -->
# คู่มือวิศวกรรม Solar Grid-Connected

คู่มือออกแบบ ตรวจสอบ และทดสอบระบบ Solar Grid-Connected สำหรับ Solar Rooftop, Solar Farm และ Industrial Solar ครอบคลุม 400 V, 22 kV และ 115 kV

> **ขอบเขตความปลอดภัย:** คู่มือนี้เป็นแนวทางวิศวกรรม ไม่ทดแทน protection coordination study, grid impact study, ข้อกำหนดผู้ผลิต หรือเงื่อนไขอนุมัติของ PEA/MEA/EGAT/ERC ที่ใช้กับโครงการจริง

## วิธีใช้

เริ่มที่ [สารบัญ](SUMMARY.md) แล้วทำตามลำดับ Survey → PCC → CT/PT → Metering/Protection → Approval → Commissioning → O&M. ค่าตัวอย่างทั้งหมดต้องตรวจทานโดยวิศวกรผู้ออกแบบก่อนใช้งานจริง.

## หลักการสำคัญ

```mermaid
flowchart LR
 U[Utility] --- PCC[PCC: จุดเชื่อมต่อร่วม]
 PCC --- M[Revenue / check metering]
 PCC --- P[Protection]
 PCC --- L[Load]
 PCC --- S[PV inverter / transformer]
 S --> C[Zero-export controller]
 C --> S
```

การวัดกำลังและการป้องกันที่ PCC ต้องคง polarity, ratio, burden และ phase association ให้ถูกต้องตลอดสายสัญญาณ.
...
ตัวอย่างโหลด 2.5 MVA ที่ 22 kV: \(I=2500/(\sqrt3×22)=65.6\) A; CT 100/1 A เป็นเพียง candidate ต้องตรวจ fault/protection study.
...
<!-- END SOURCE: 04_PT_VT_CVT.md -->
<!-- SOURCE: 05_CT_Burden.md -->
...
\[S_{burden}=I_s^2 Z_{total}\quad [VA]\]
...
\[R_{loop}=\rho\frac{2L}{A}\quad;\quad \rho=0.0175\ \Omega\,mm^2/m\]\n+
...
flowchart LR
 ...
 F --> E[Ratio/phase/composite error เพิ่ม]
 E --> M[Meter หรือ relay เห็นกระแสต่ำ/เพี้ยน]
```
...
\[R_{cable}=0.0175\times300/4=1.31\ \Omega\]

| รายการ | CT 400/1 A | CT 400/5 A |
|---|---:|---:|
| Cable burden | (1^2×1.31=1.31\) VA | (5^2×1.31=32.75\) VA |
...
1. Available cable burden = (0.70×15-1.0=9.5\) VA
2. ที่ 1 A, allowable loop resistance = (9.5/1^2=9.5\ \Omega)
3. (A_{min}=0.0175×(2×200)/9.5=0.737\ mm^2)
...
<!-- END SOURCE: 05_CT_Burden.md -->
<!-- SOURCE: 06_CT_Saturation.md -->
# 6. CT Saturation
...
```mermaid
flowchart TD
 F[Fault / hi…1612 tokens truncated…ับ measuring relays/protection และรวมบริบท control, monitoring, communication และ process interface [IEC 60255-1](https://webstore.iec.ch/en/publication/59762). EMC และ safety ของอุปกรณ์เป็นคนละมิติที่ต้องตรวจตามมาตรฐาน/ผู้ผลิตด้วย.

<!-- END SOURCE: 08_Protection.md -->
<!-- SOURCE: 09_Reverse_Power.md -->
...
<!-- END SOURCE: 09_Reverse_Power.md -->
<!-- SOURCE: 10_Zero_Export.md -->
...
<!-- END SOURCE: 10_Zero_Export.md -->
<!-- SOURCE: 11_400V_Design.md -->
# 11. การออกแบบ 400 V
...
คำนวณจาก \(I=P/(\sqrt3Vpf)\). ตรวจ busbar/cable derating, fault level, ACB breaking capacity, earthing, anti-islanding, metering CT และ utility interface ก่อนเลือกอุปกรณ์.
...
<!-- END SOURCE: 11_400V_Design.md -->
<!-- SOURCE: 12_22kV_Design.md -->
# 12. การออกแบบ 22 kV
...
ตรวจ insulation level, CT/PT cores, relay coordination, transformer vector group, cable ampacity, switching duty, arc-flash และ approved protection interface. ตัวอย่าง 5 MW: \(5000/(\sqrt3×22)=131.2\) A.
...
<!-- END SOURCE: 12_22kV_Design.md -->
<!-- SOURCE: 13_115kV_Design.md -->
# 13. การออกแบบ 115 kV
...
ตัวอย่าง 60 MVA: \(60,000/(\sqrt3×115)=301\) A. ต้องทำ grid impact, insulation coordination, protection/teleprotection และ bay design ตามเจ้าของระบบ.
...
<!-- END SOURCE: 13_115kV_Design.md -->
<!-- SOURCE: 14_Utility_Requirements.md -->
# 14. ข้อกำหนด Utility
...
<!-- END SOURCE: 14_Utility_Requirements.md -->
<!-- SOURCE: 15_Approval_Process.md -->
...
<!-- END SOURCE: 15_Approval_Process.md -->
<!-- SOURCE: 16_Commissioning.md -->
# 16. Commissioning
...
<!-- END SOURCE: 16_Commissioning.md -->
<!-- SOURCE: 17_Operation_Maintenance.md -->
...
<!-- END SOURCE: 17_Operation_Maintenance.md -->
<!-- SOURCE: 18_Case_Studies.md -->
# 18. กรณีศึกษา
...
<!-- END SOURCE: 18_Case_Studies.md -->
<!-- SOURCE: 19_Checklists.md -->
...
<!-- END SOURCE: 19_Checklists.md -->
<!-- SOURCE: 20_References.md -->
